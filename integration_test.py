...
def pytest_generate_tests(metafunc):
    if 'game' in metafunc.fixturenames and 'play' in metafunc.fixturenames:
        game_date = datetime.fromisoformat(metafunc.config.getoption("--game_date"))
        print(game_date)
        metafunc.parametrize( "game, play", [(game_id, play_by_play) for game_id in get_game_ids(game_date) for play_by_play in get_play_by_play(game_id)])
